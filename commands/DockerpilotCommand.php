...
class DockerpilotCommand extends Command
{
    ...
    {
        $process = new Process("docker service ls --filter=\"name=" . $name . "_\" -q");
    ...
    /**
     * Get Docker container ID by name and status.
     *
     * @param $name
     * @return string
     * @throws Exception
     */
    protected function getDockerContainerID($name, $status = 'running')
    {
        $process = new Process("docker ps -f \"name=$name\" -f \"status=$status\" -q");
    ...
    public function askForApp($input, $output, $questionName, $state = false, $stack = false)
    {
        ...
        if (count($apps) > 0) {
            ...
            $this->appDir = dp_path( $server['storagePath'] . '/config/apps/' . $this->app);

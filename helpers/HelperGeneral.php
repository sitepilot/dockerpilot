...
function dp_get_app_config($appDir, $key = 'all') {
    ...
    $defaultConfigFile = SERVER_WORKDIR . '/stacks/app/config.default.yml';
...
function dp_get_apps()
{
    ...
    $appsPath = $server['storagePath'] . '/config/apps';

    $dirs = array_filter(glob( $appsPath . '/*'), 'is_dir');
    $apps = array();
    foreach ($dirs as $dir) {
        $apps[dp_path($dir)] = str_replace($appsPath . '/', '', $dir);
    }
    if (count($apps) > 0) {
        return $apps;
    }
